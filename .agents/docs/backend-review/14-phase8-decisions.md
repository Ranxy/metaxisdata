...
| 批 4 · 性能与整洁 | **✅ 已完成（部分转遗留）**：`D1`/`D2`/`D5`/`D6`（2/4）/`D11`/`D12`/`D13`/`D18`/`D19` 与 `C2`/`C3`（`6baef9a` `1d4459b` `40256b1` `2de85ee` `6384d37` `756bcf9` `68aae4b`）；`D17` 判定为误报；`D3`/`D4`/`D6` 两项/`D7`（刻意不做）/`D8`/`D9`/`D10`/`D20` 转遗留，理由见 `13` 第八节「批 4 实施记录」 |
| 批 5 · 测试与 CI | **✅ 已完成**：`E1`–`E13` 与 `D22`（`f950059` `98c3db4` `a5214dc` `a9f8e7b` `18638ba` `f4b7baf` `6f96432` `b88515b`）；`E7`（CI 实跑）、`E12`/`E13`（大范围覆盖率缺口）、`E3` 的启动 fixture 删除、`E6` 的覆盖率阈值、`E9` 的 testify 迁移转遗留，见 `13` 第八节「批 5 实施记录」 |
| 批 6 · 依赖决策的收尾 | **✅ 已完成**：`B13`（收 COLUMN 声明）、`C11`（域白名单）、`C14`（provider 允许列表 + 选择器）与 F4/F6/F7 文档（`57feb35` `b975a1d` `38dd450`）；两处设置字段**不需要 schema 增量**（落在既有 `WORKSPACE_PROFILE` 的 protojson 里），见 `13` 第八节「批 6 实施记录」 |
