...
- `metadata` 搜索仍是有意的子串匹配（trgm 索引服务），LLM profile 仍是 30s TTL 缓存 + 写侧失效，`openlineage_run` 默认永久保留。
