...
| 批次 | 步骤 | 状态 | 提交 |
| --- | --- | --- | --- |
| A · 安全 | A1 token 吊销加固（`02 H1`；吊销缓存仍为进程内，见下"已知取舍"） | ✅ | `976ebc5` |
...
- **阶段 7 之后**各模块报告里仍未修复的条目已重新逐条核对并汇总到 [`13-remaining-work.md`](13-remaining-work.md)（去重后 74 项 + 10 个待确认决策 + 阶段 8 建议批次）；旧报告的 `✅/◐/⏳` 标记过期严重，不要直接沿用。10 项决策已全部关闭（[`14-phase8-decisions.md`](14-phase8-decisions.md)），**阶段 8 的批 1–3、5、6 与批 4（部分）均已完成**；批 4 中 `D3`/`D4`/`D6` 两项/`D10`/`D20` 等 Low/Debt 项转为遗留（`D7`/`D8`/`D9` 为刻意不做或已接受设计，`D17` 经实测判定为误报）。完整进度、逐项提交与遗留理由见 `13` 开头与第八节各批「实施记录」。
