...
- **阶段 7 之后**各模块报告里仍未修复的条目已重新逐条核对并汇总到 [`13-remaining-work.md`](13-remaining-work.md)（去重后 74 项 + 10 个待确认决策 + 阶段 8 建议批次）；旧报告的 `✅/◐/⏳` 标记过期严重，不要直接沿用。10 项决策已全部关闭（[`14-phase8-decisions.md`](14-phase8-decisions.md)），**阶段 8 批 1（契约与文档一致性，14 项）、批 2（正确性，12 项）、批 3（安全收尾，5 项）与批 6（决策后的实现，3 项）均已完成**；仅剩批 4（性能与整洁）与批 5（测试与 CI）。进度与逐项提交见 `13` 开头与第八节各批「实施记录」。
