...
- **阶段 7 之后**各模块报告里仍未修复的条目已重新逐条核对并汇总到 [`13-remaining-work.md`](13-remaining-work.md)（去重后 74 项 + 10 个待确认决策 + 阶段 8 建议批次）；旧报告的 `✅/◐/⏳` 标记过期严重，不要直接沿用。10 项决策已全部关闭（[`14-phase8-decisions.md`](14-phase8-decisions.md)），**阶段 8 批 1（契约与文档一致性）、批 2（正确性）、批 3（安全收尾）、批 5（测试与 CI）与批 6（决策后的实现）均已完成**；仅剩批 4（性能与整洁）。进度、逐项提交与本批遗留见 `13` 开头与第八节各批「实施记录」。
