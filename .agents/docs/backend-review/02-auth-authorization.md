...
- `backend/api/auth/auth.go:144-159`：JWT 解析未使用 `WithValidMethods/WithIssuer/WithExpirationRequired`，`issuer` 常量只写不校验。
...
- **被禁用的授权**：`grpc_routes.go:85` 引用的 `NewACLInterceptor`/`iamManager` 已不存在；`common.AuthContext` 的 `Permission`/`AuthMethod`/`Resources`、`HasWorkspaceResource`、`GetProjectResources` 无消费者；`store.RoleMessage.Permissions` 被填充但从不校验。
- **空实现**：`user_service.go:788-790` 与 `auth_service.go:353-355` 两个同名 `userCountGuard` 都是 `return nil`；`getActiveUserCount` 与 `store.CountActiveUsers` 功能重复。
...
- **注释掉的代码块**：注册/许可校验与 metric 上报（`user_service.go:291-310,400-412`）、调用者身份校验（`user_service.go:478-480`）、export format 转换（`common.go:362-390`）、`GetUsersByRoleInIAMPolicy`（`utils/member.go:26-68`）。
