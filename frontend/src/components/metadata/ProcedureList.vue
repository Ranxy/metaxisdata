<template>
  <div>
    ...
    <Table v-else>
      <TableHeader>
        <TableRow>
          <TableHead>{{ t("metadataBrowser.procedureName") }}</TableHead>
...
const rows = computed(() => {
  const list: Array<{ stored: StoredMetadata; value: ProcedureMetadata }> = [];

  for (const stored of props.items) {
    if (stored.type.case === "procedureMetadata") {
