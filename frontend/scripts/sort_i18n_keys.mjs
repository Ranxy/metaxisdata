// frontend/scripts/sort_i18n_keys.mjs
//
// Normalizes every locale catalog under src/locales so that object keys are
// sorted recursively and the file is formatted as two-space JSON with a
// trailing newline. Sorting keeps diffs small and makes the en-US / zh-CN
// files easy to compare side by side.
//
//   node frontend/scripts/sort_i18n_keys.mjs          # rewrite in place
//   node frontend/scripts/sort_i18n_keys.mjs --check   # fail if not sorted
//
// The pure helpers are exported so Vitest can exercise them without touching
// real catalog files.
