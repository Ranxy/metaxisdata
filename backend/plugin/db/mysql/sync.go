...
func (d *Driver) reconcileViewDefinition(ctx context.Context, databaseName, viewName string) (string, error) {
	...
	if err != nil {
		slog.Warn("failed to get view definition", slog.String("viewName", viewName), slog.String("databaseName", databaseName), log.WithError(err))
		return "", nil
	}
	...
	for tableKey, partitions := range result {
		...
		for showRows.Next() {
			...
			if err := showRows.Scan(&tableName, &createTable); err != nil {
				slog.Warn("failed to scan row", slog.String("query", showQuery), log.WithError(err))
			}
			...
			if partitionRegexp.MatchString(createTable) {
				...
				if err != nil {
					slog.Warn("failed to parse partition number", slog.String("query", showQuery), log.WithError(err))
				}
			}
			if subPartitionRegexp.MatchString(createTable) {
				...
				if err != nil {
					slog.Warn("failed to parse subpartition number", slog.String("query", showQuery), log.WithError(err))
				}
			}
		...
		if err := showRows.Err(); err != nil {
			slog.Warn("rows err", slog.String("query", showQuery), log.WithError(err))
		}
		// nolint
		if err := showRows.Close(); err != nil {
			slog.Warn("failed to close row", slog.String("query", showQuery), log.WithError(err))
		}
	}
