...
const autoIncrementSymbol = "AUTO_INCREMENT"
...
// read-only `sys` metadatabase; Doris has no `sys`.
...
func (d *Driver) SyncInstance(ctx context.Context) (*db.InstanceMetadata, error) {
	...
	if err != nil {
		slog.Debug("failed to get lower_case_table_names variable", log.WithError(err))
	} else {
		v, err := strconv.ParseInt(lowerCaseTableNamesText, 10, 32)
		if err != nil {
			slog.Debug("failed to parse lower_case_table_names variable", log.WithError(err))
		} else {
			lowerCaseTableNames = int32(v)
		}
	}

	// Query db info. Only SCHEMA_NAME is read: this catalog returns
	// NUL-terminated charset/collation values that the JSONB store rejects, and
	// neither engine has a meaningful database-level charset.
...
// isSyncRollup reports whether a StarRocks information_schema.materialized_views
// row describes a synchronous rollup rather than a standalone (async)
// materialized view. Sync rollups are rollup indexes on the base table: they
// appear in that catalog with REFRESH_TYPE='ROLLUP' but have no
// information_schema.tables row, so they are excluded from the
// materialized-view set. Only the confirmed 'ROLLUP' value is excluded; async
// refresh types and any unknown/empty value are kept.
func isSyncRollup(refreshType string) bool {
	return refreshType == "ROLLUP"
}

// isMaterializedView reports whether a table row is a materialized view, based
// on its table type and membership in the materialized-view set. StarRocks
// reports materialized views with TABLE_TYPE='VIEW', Doris with 'BASE TABLE',
// and some setups with 'MATERIALIZED VIEW'; in every case, presence in
// materializedViewMap is the authoritative signal.
...
// getMaterializedViews returns the materialized views of the current database
// keyed by table name. Doris and StarRocks expose them through different
// catalogs, so the query and projection are engine-specific. The query is
// best-effort: an engine/version that does not support it is logged and yields
// an empty set rather than failing the sync.
func (d *Driver) getMaterializedViews(ctx context.Context) (map[db.TableKey]*storepb.MaterializedViewMetadata, error) {
	...
	case storepb.Engine_DORIS:
		// Doris exposes materialized views via the mv_infos() table-valued
		// function: https://doris.apache.org/docs/sql-manual/sql-functions/table-valued-functions/mv_infos
		query = `SELECT Name, QuerySql FROM mv_infos("database"=?)`
	case storepb.Engine_STARROCKS:
		// StarRocks exposes materialized views via
		// information_schema.materialized_views. REFRESH_TYPE separates async
		// materialized views from synchronous rollups, which are excluded.
		// IFNULL guards the bare-string scans: database/sql errors on a
		// NULL->string scan.
		query = `SELECT TABLE_NAME, IFNULL(REFRESH_TYPE, ''), IFNULL(MATERIALIZED_VIEW_DEFINITION, '') FROM information_schema.materialized_views WHERE TABLE_SCHEMA = ?`
	default:
	...
	if err != nil {
		...
		slog.Debug("failed to query materialized views, might not be supported in this version", log.WithError(err))
		return materializedViewMap, nil
	}
	defer rows.Close()
...
func (d *Driver) SyncDBSchema(ctx context.Context) (*storepb.DatabaseSchemaMetadata, error) {
	...
	for columnRows.Next() {
		...
		setColumnMetadataDefault(column, defaultStr, isNullBool, extra)
	...
	for tableRows.Next() {
		...
		key := db.TableKey{Schema: "", Table: tableName}
		// StarRocks reports materialized views as TABLE_TYPE='VIEW', Doris as
		// 'BASE TABLE'; route any row whose name is in the materialized-view set
		...
		case baseTableType:
			...
			// TODO: add index information whenever it is available.
		...
		default:
			// Includes MATERIALIZED VIEW rows absent from the materialized-view
			// set and any other unrecognized type.
	...
	// Confirm the database still exists; it may have been dropped since the last
	// instance sync. Only the name is read because this catalog returns
	// NUL-terminated charset/collation values that the JSONB store rejects.
	databaseQuery := `
		SELECT
			SCHEMA_NAME
		FROM information_schema.SCHEMATA
	...
	if err := d.db.QueryRowContext(ctx, databaseQuery, d.databaseName).Scan(&databaseName); err != nil {
		if err == sql.ErrNoRows {
...
// setColumnMetadataDefault derives the stored default for a StarRocks/Doris
// column. Unlike MySQL there is no ON UPDATE support, so only the default value
// is handled; the catalog returns it unquoted.
func setColumnMetadataDefault(column *storepb.ColumnMetadata, defaultStr sql.NullString, isNullBool bool, extra string) {
	if defaultStr.Valid {
		unquotedDefault := dbmysql.UnquoteMySQLString(defaultStr.String)
		switch {
		case dbmysql.IsCurrentTimestampLike(unquotedDefault):
			// CURRENT_TIMESTAMP, CURRENT_DATE, etc. are function names, not
			// expressions.
			column.Default = unquotedDefault
		case strings.Contains(extra, "DEFAULT_GENERATED"):
			// Expression defaults like DEFAULT (uuid()).
			unescapedDefault := dbmysql.UnescapeExpressionDefault(unquotedDefault)
			column.Default = fmt.Sprintf("(%s)", unescapedDefault)
		default:
			// Static defaults like DEFAULT 'hello' or DEFAULT 42. Preserve
			// quotes for dump compatibility.
			column.Default = defaultStr.String
		}
	} else if strings.Contains(strings.ToUpper(extra), autoIncrementSymbol) {
		// Use the upper case to be consistent with the dump output.
		column.Default = autoIncrementSymbol
	} else if isNullBool {
		// This is NULL if the column has an explicit default of NULL, or if the
