...
func getChecks(txn *sql.Tx, schemas []string) (map[db.TableKey][]*storepb.CheckConstraintMetadata, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&parentObjectID, &checkName, &definition); err != nil {
			return nil, err
		}
...
func getForeignKeys(txn *sql.Tx, schemas []string) (map[db.TableKey][]*storepb.ForeignKeyMetadata, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&schemaName, &tableName, &fkName, &referencedSchemaName, &referencedTableName, &onDelete, &onUpdate, &parentColumnName, &referencedColumnName); err != nil {
			return nil, err
		}
...
func getViews(txn *sql.Tx, columnMap map[db.TableKey][]*storepb.ColumnMetadata) (map[string][]*storepb.ViewMetadata, error) {
	...
	// Collect the objects each view references. The migration DDL renderer uses
	// this to order view drops and creates, so it never has to parse the view
	// definition itself.
	dependencySet := make(map[int]map[string]*storepb.DependencyColumn)
	dependenciesQuery := `
	SELECT
		d.referencing_id,
	...
	dependencyRows, err := txn.Query(dependenciesQuery)
	if err != nil {
		return nil, util.FormatErrorWithQuery(err, dependenciesQuery)
	}
	defer dependencyRows.Close()
	for dependencyRows.Next() {
		var objectID int
		var schemaName, tableName sql.NullString
		if err := dependencyRows.Scan(&objectID, &schemaName, &tableName); err != nil {
			return nil, err
		}
		if !tableName.Valid {
			continue
		}
		if _, ok := dependencySet[objectID]; !ok {
			dependencySet[objectID] = make(map[string]*storepb.DependencyColumn)
		}
		dependency := &storepb.DependencyColumn{
			Schema: schemaName.String,
			Table:  tableName.String,
		}
		dependencySet[objectID][formatDependencyKey(schemaName.String, tableName.String)] = dependency
	}
	...
	for rows.Next() {
		...
		if objectID.Valid {
			view.DependencyColumns = sortedDependencyColumns(dependencySet[int(objectID.Int32)])
		}
...
func formatDependencyKey(schemaName, tableName string) string {
	return schemaName + "." + tableName
}

// sortedDependencyColumns returns the dependency columns in a stable order so
// the stored metadata hash does not change between syncs.
func sortedDependencyColumns(dependencies map[string]*storepb.DependencyColumn) []*storepb.DependencyColumn {
	...
	slices.Sort(keys)
	result := make([]*storepb.DependencyColumn, 0, len(keys))
	for _, key := range keys {
		result = append(result, dependencies[key])
	}
	return result
}
