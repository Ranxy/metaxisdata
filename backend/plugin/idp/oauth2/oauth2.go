...
func (p *IdentityProvider) ExchangeToken(ctx context.Context, redirectURL, code string) (string, error) {
	...
	if err != nil {
		slog.Error("Failed to exchange access token", slog.String("code", code), log.WithError(err))
...
func (p *IdentityProvider) UserInfo(token string) (*storepb.IdentityProviderUserInfo, map[string]any, error) {
	...
	if err != nil {
		slog.Error("Failed to get user information", slog.String("token", token), log.WithError(err))
		return nil, nil, errors.Wrap(err, "failed to get user information")
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		slog.Error("Failed to read response body", slog.String("token", token), log.WithError(err))
		return nil, nil, errors.Wrap(err, "failed to read response body")
	}

	var claims map[string]any
	if err := json.Unmarshal(body, &claims); err != nil {
		slog.Error("Failed to unmarshal response body", slog.String("token", token), slog.String("body", string(body)), log.WithError(err))
