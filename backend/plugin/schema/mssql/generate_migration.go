...
func generateMigration(diff *schema.MetadataDiff) (string, error) {
	...
	// Comment changes on existing objects are reported by the differ as
	// CommentChanges, so they are rendered from there instead of being derived a
	// second time from the table diff.
	for _, commentDiff := range diff.CommentChanges {
		...
		case schema.CommentObjectTypeTable:
			commentSQL = generateTableCommentSQL(commentAction(commentDiff), commentDiff.SchemaName, commentDiff.ObjectName, commentDiff.NewComment)
		case schema.CommentObjectTypeColumn:
			// A column comment object name is "<table>.<column>".
			separator := strings.LastIndex(commentDiff.ObjectName, ".")
			if separator < 0 {
				continue
			}
			tableName := commentDiff.ObjectName[:separator]
			columnName := commentDiff.ObjectName[separator+1:]
			commentSQL = generateColumnCommentSQL(commentAction(commentDiff), commentDiff.SchemaName, tableName, columnName, commentDiff.NewComment)
		default:
			continue
		}
		_, _ = buf.WriteString(commentSQL)
...
// commentAction maps a comment change onto the extended property action.
func commentAction(commentDiff *schema.CommentDiff) string {
	switch {
	case commentDiff.OldComment == "" && commentDiff.NewComment != "":
		return "ADD"
	case commentDiff.OldComment != "" && commentDiff.NewComment == "":
		return "DROP"
	default:
...
func dropViewsInOrder(diff *schema.MetadataDiff, buf *strings.Builder) {
	...
	for _, viewDiff := range viewsToProcess {
		...
		// The edge points from the dependent view to the view it depends on, so
		// the reverse topological order drops the dependents first.
		if viewDiff.OldView != nil {
			for _, dep := range viewDiff.OldView.DependencyColumns {
				depID := getObjectID(dep.Schema, dep.Table)
				// Only add an edge if the dependency is also being dropped/altered.
				if _, exists := viewMap[depID]; exists {
...
func createViewsInOrder(diff *schema.MetadataDiff, buf *strings.Builder) {
	...
	for _, viewDiff := range viewsToProcess {
		...
		// The edge points from the dependency to the dependent view, so the
		// dependency is created first.
		if viewDiff.NewView != nil {
			for _, dep := range viewDiff.NewView.DependencyColumns {
				depID := getObjectID(dep.Schema, dep.Table)
				// Only add an edge if the dependency is also being created/altered.
				if _, exists := viewMap[depID]; exists {
...
func generateSpatialIndexDDL(index *storepb.IndexMetadata, schemaName, tableName string) string {
	...
	if index.SpatialConfig == nil || index.SpatialConfig.Tessellation == nil {
		...
		_, _ = buf.WriteString("Please verify BOUNDING_BOX, GRIDS, and other parameters manually. */")
		return buf.String()
	}
